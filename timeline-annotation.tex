...
\tikzset{
  ...
  relation label/.style={edge node={node[auto, font=\scshape]{#1}}},
}
...
\strut\alert{
\only<2>{TimeBank \smallcite{pustejovsky2003timebank}: salient relations}%
\only<3>{TempEval \smallcite{verhagen-EtAl:2010:SemEval}: frequent pairs}%
\only<4->{Dense Timebank \smallcite{cassidy-etal:2014:ACLShort}: all pairs in adjacent sentences}}
